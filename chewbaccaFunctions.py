...
def assemble_pear(args, pool=Pool(processes=1), debug=False):
    ...
    try:
        ...
        rslt = parallel(runProgramRunnerInstance, [ProgramRunner("pear", [forwards, reverse,
                                                           "%s/%s_%s" % (args.outdir, args.name, getFileName(forwards)),
                                                                   args.threads],
                                                          {"exists": [forwards, reverse]})
                                            for forwards, reverse in inputs], pool, debug)
...
def splitOnBarcodes(args, pool=Pool(processes=1), debug=False):
    ...
    try:
        ...
        parallel(runProgramRunnerInstance, [ProgramRunner("barcode.splitter",
                                           ...
                                           "_%d_splitOut.fastq" % id_],
                                                  {"exists": [input_]}) for input_, id_ in file_id_pairs], pool, debug)
...
def renameSequences(args, pool=Pool(processes=1), debug=False):
...
def trim_flexbar(args, pool=Pool(processes=1), debug=False):
...
def trimmomatic(args, pool=Pool(processes=1), debug=False):
...
def dereplicate(args, pool=Pool(processes=1), debug=False):
    """Uses a sliding window to identify and trim away areas of low quality.
...
def align_mothur(args, pool=Pool(processes=1), debug=False):
...
def partition(args, pool=Pool(processes=1), debug=False):
...
def merge(args, pool=Pool(processes=1), debug=False):
...
def ungapFasta(args, pool=Pool(processes=1), debug=False):
    """Clusters sequences.
       :param args: An argparse object with the following parameters:
...
def cluster(args, pool=Pool(processes=1), debug=False):
...
def queryBiocode(args, pool=Pool(processes=1), debug=False):
...
def queryNCBI(args, pool=Pool(processes=1), debug=False):
    """Compare reference sequences to the fasta-formatted query sequences, using global pairwise alignment.
...
def prescreen(args, pool=Pool(processes=1), debug=False):
...
#TODO doc and debug
def minhash(args, pool=Pool(processes=1), debug=False):
