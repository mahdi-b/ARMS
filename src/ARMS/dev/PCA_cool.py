...
def encode_fasta(input_file, k):
    ...
    for seq in SeqIO.parse(open(input_file, 'r'), 'fasta'):
        ...
        names.append(name[0])
        try:
            tax.append(name[1].split(",")[0].split(":")[1])
        except:
            tax.append(name[1].split(",")[0].split(":")[1])
    ...
    return data, names, tax


def do_pca(input_file, k, dim, graph=False):
    data, names, tax = encode_fasta(input_file, k)
    ...
    reduced_dat.loc[:, 'tax'] = Series(tax, index=reduced_dat.index)

    reduced_dat = reduced_dat.loc[reduced_dat['tax'] != "Arthropoda"]



    # print reduced_dat.sort_values(cols)
    reduced_dat.to_csv(path_or_buf="%s.dataframe" % input_file.split(".")[0], sep="\t", columns= ["tax"]+cols,
                       index=True, header=True)
    tax_set = set(reduced_dat["tax"])
    n_tax = len(tax_set)
    colors =  [name for name, hex in matplotlib.colors.cnames.iteritems()]
    col_map = {key: value for (key, value) in zip(tax_set, colors[:n_tax])}
    coloring = [col_map[tax] for tax in reduced_dat["tax"]]
    print n_tax
    if graph:
        #=================
        #== Plot 2d ======
        ...
        if dim == 2:
            ...
            print "Graphing..."
            ax = sns.lmplot(x='x', y='y',data=reduced_dat, hue="tax", fit_reg=False)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        do_pca(sys.argv[1], k=5, dim=3,graph=True)
