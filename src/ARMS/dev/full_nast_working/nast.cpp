...
Nast::Nast(string cand_ref, string cand_aln, string temp_ref, string temp_aln) {
	try {
		maxInsertLength = 0;
		// Create sequences for cand_ref, temp_ref
		templateSeq = new Sequence("temp", temp_ref);
		candidateSeq = new Sequence("cand", cand_ref);
		init(cand_aln, temp_aln);	//	This is part A in Fig. 2 of DeSantis et al.
...
void Nast::init(string cand_aln, string temp_aln){	//	Here we call one of the pairwise alignment methods to align our unaligned candidate
								//	and template sequences
	try {	
		
		string candAln = cand_aln;
		string tempAln = temp_aln;
...
//				i -= insertLength;
				...
				if (i < 0) { cout << ("i is negative");  } 
			} 
		}
	}
	catch(exception& e) {
		cout << "removeExtraGaps";
		exit(1);
	}	
}
	...
	//cout << pairwiseAlignIndex << '\t' << fullAlignIndex << '\t' << pairwiseLength << endl;
			if(isalpha(tempPair[pairwiseAlignIndex]) && isalpha(tempAln[fullAlignIndex])
			   && isalpha(candPair[pairwiseAlignIndex])){
				...
				cout << "We're into D " << fullAlignIndex << " " <<  pairwiseAlignIndex; 
				pairwiseAlignIndex++;
			}
			else{
				...
				cout << "We're into F " <<  fullAlignIndex << " " <<  pairwiseAlignIndex; 
				pairwiseAlignIndex++;
				fullAlignIndex++;			
			}		
		}
...
int i = 0;
if (argc <5) {
	cout << "\nUsage: unaligned_query pairwise_query MSA_ref_template pairwise_ref\n\n";
	exit(0);
}
//string cand_ref, string cand_aln, string temp_ref, string temp_aln
for (i=0; i<5; i++) {
	cout << argv[i] << "\n";
}
Nast n = Nast(argv[1], argv[2], argv[3], argv[4]);
cout <<"\n\n";
cout << "MSA template:\n";
cout << argv[3] << "\n\n";
cout << "Aligned candidate:\n";
cout << n.candidateSeq->getAligned() << "\n";

//cout << n.templateSeq->getAligned() << "\n";
return 0;
}
