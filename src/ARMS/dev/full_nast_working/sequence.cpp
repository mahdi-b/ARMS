...
Sequence::Sequence(istringstream& fastaString){
	try {
		...
		if (!control_pressed) { 
			...
			if ((numAmbig / (float) numBases) > 0.25) { cout << ("[WARNING]: We found more than 25% of the bases in sequence " + name + " to be ambiguous. Mothur is not setup to process protein sequences.");  }
		}
...
string Sequence::getSequenceName(ifstream& fastaFile) {
	try {
        ...
        }else{ cout << "Error in reading your fastafile, at position " << fastaFile.tellg() << ". Blank name.";  control_pressed = true;  }
        
		return name;
	}
	catch(exception& e) {
		cout << ("getSequenceName");
		exit(1);
	}
}
//********************************************************************************************************************
...
int Sequence::filterToPos(int start){
    ...
    if (start > aligned.length()) { start = aligned.length(); cout << ("[ERROR]: start to large.\n"); }
...
int Sequence::filterFromPos(int end){
    ...
    if (end > aligned.length()) { end = aligned.length(); cout << ("[ERROR]: end to large.\n"); }
