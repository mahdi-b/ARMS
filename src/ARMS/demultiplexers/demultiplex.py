...
def demultiplex_main(input_f, barcodes, outdir, program, threads, aux_params):
    ...
    :param threads: Number of processes to use to demultiplex input files.
    :param aux_params: Dictionary of program-specific named-parameters.
    """
    makeDirOrdie(outdir)
    # Get input files
    ...
    if program == "fastx":
        demultiplex_fastx(file_id_pairs, barcodes, outdir, threads, pool)
    cleanup_pool(pool)


def demultiplex_fastx(file_id_pairs, barcodes, outdir, threads, pool):
